...
function normalize(v) { return trimVal(v).toLowerCase(); }
...
function populateRangeList(selectedSupplierName) {
  if (!rangeList) return;
  rangeList.innerHTML = "";
  ...
  if (selectedSupplierName) {
    const ranges = metaRangesBySupplier.get(selectedSupplierName) || [];
    const seen = new Set();
...
function renderResults(items, groupBySupplier=false) {
    ...
    for (const it of items) {
      if (groupBySupplier) {
        ...
        if (sup && sup !== lastSup) {
          ...
          tdG.colSpan = DISPLAY_COLS.length;
          const supMeta = metaSuppliers.find(x => x.SupplierName === sup);
          tdG.textContent = (supMeta && supMeta.SupplierDebrand) ? `${supMeta.SupplierName} — ${supMeta.SupplierDebrand}` : sup;
          trG.appendChild(tdG);
  ...
  if (resultsFooter) resultsFooter.textContent = `Showing ${items.length} results (use Supplier + Range + Search to narrow).`;
}

async function refreshResults() {
  const supplier = parseLabelValue(supplierInput?.value || "");
  const range = parseLabelValue(rangeInput?.value || "");
  const q = normalize(searchBox?.value || "");

  if (!supplier && !range) {
    renderResults([]);
    setStatus("Type a supplier or a range to search. (If you haven’t imported data yet, tap Import / Replace.)");
    return;
  }
...
function showHelp() {
  alert(
`Tips:
- Type in Supplier or Range (no scrolling).
- Range-only search groups results by supplier.
- Tap a row to view full details.

Import:
Tap Import / Replace and choose your XLSX (Sheet2).`
  );
}
...
function buildHeaderMap(rows) {
  ...
  for (const col of DISPLAY_COLS.concat(HIDDEN_COLS)) {
...
async function importXlsxFile(file) {
  if (!window.XLSX) {
    alert("XLSX parser didn’t load. Open once while online, then try again.");
    return;
  }
...
async function init() {
  ...
  if (detailOverlay) detailOverlay.addEventListener("click", (e)=>{ if (e.target === detailOverlay) closeDetailOverlay(); });
  ...
  if (supplierInput) {
    supplierInput.addEventListener("input", onSupplierChanged);
    supplierInput.addEventListener("change", onSupplierChanged);
  }
  if (rangeInput) {
    rangeInput.addEventListener("input", refreshResults);
    rangeInput.addEventListener("change", refreshResults);
  }
